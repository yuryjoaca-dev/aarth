      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-16 pb-10">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-12 lg:gap-8">
          ...
          <div className="lg:col-span-2">
            ...
            <a href="https://network.procore.com/p/aarth-construction-inc-edmonton" rel="noopener noreferrer" target="_blank"
              className="inline-block opacity-90 hover:opacity-100 transition-opacity duration-200">
              <img src="/procore-badge.svg" alt="Verified on Procore" className="h-12 w-auto" />
            </a>
          </div>

          <div>
            <h3 className="text-white font-medium text-[10px] uppercase tracking-widest mb-5" style={{ fontFamily: 'Inter, sans-serif' }}>Quick Links</h3>
            <ul className="space-y-3">
              {quickLinks.map((link) => (
                <li key={link.href}>
                  <Link href={link.href} className="text-sm text-slate-400 hover:text-[#C9963B] transition-colors duration-150 flex items-center gap-2 group font-light" style={{ fontFamily: 'Inter, sans-serif' }}>
          ...
          <div>
            <h3 className="text-white font-medium text-[10px] uppercase tracking-widest mb-5" style={{ fontFamily: 'Inter, sans-serif' }}>Services</h3>
            <ul className="space-y-3">
              {services.map((s) => (
                <li key={s.href}>
                  <Link href={s.href} className="text-sm text-slate-400 hover:text-[#C9963B] transition-colors duration-150 flex items-center gap-2 group font-light" style={{ fontFamily: 'Inter, sans-serif' }}>
                    <span className="w-1 h-1 bg-[#C9963B] flex-shrink-0 opacity-60 group-hover:opacity-100 transition-opacity" />
                    {s.label}
                  </Link>
                </li>
          ...
          <div>
            <h3 className="text-white font-medium text-[10px] uppercase tracking-widest mb-5" style={{ fontFamily: 'Inter, sans-serif' }}>Contact Us</h3>
            <ul className="space-y-4">
              <li className="flex gap-3">
                <MapPin className="w-4 h-4 text-[#C9963B] flex-shrink-0 mt-0.5" />
                <a href="https://www.google.com/maps?q=16307+111+Ave+NW,+Edmonton,+AB+T5M+2S2" target="_blank" rel="noopener noreferrer"
                  className="text-sm text-slate-400 hover:text-[#C9963B] transition-colors leading-relaxed font-light" style={{ fontFamily: 'Inter, sans-serif' }}>
                  16307 111 Ave NW,<br />Edmonton, AB, Canada
                </a>
              </li>
              <li className="flex gap-3">
                <Phone className="w-4 h-4 text-[#C9963B] flex-shrink-0 mt-0.5" />
                <a href="tel:5875962793" className="text-sm text-slate-400 hover:text-[#C9963B] transition-colors font-light" style={{ fontFamily: 'Inter, sans-serif' }}>587-596-2793</a>
              </li>
              <li className="flex gap-3">
      ...
      <div className="border-t border-white/8">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-5 flex flex-col sm:flex-row items-center justify-between gap-3">
          ...
          <div className="flex gap-5 text-xs text-slate-500 font-light" style={{ fontFamily: 'Inter, sans-serif' }}>
            <Link href="/privacy-policy" className="hover:text-[#C9963B] transition-colors">Privacy Policy</Link>
            <Link href="/terms" className="hover:text-[#C9963B] transition-colors">Terms & Conditions</Link>
          </div>
        </div>
      </div>
    </footer>
