      ...
      <section className="relative min-h-screen flex items-center justify-center overflow-hidden bg-[#080808]">
        ...
        <motion.div initial="hidden" animate="show" variants={stagger(0.12)} className="relative z-10 max-w-2xl mx-auto px-6 text-center">
          ...
          <motion.div variants={fadeUp} className="flex flex-col sm:flex-row gap-3 justify-center">
            <Link href="/" className="inline-flex items-center justify-center gap-2 bg-[#C9963B] hover:bg-[#A67A2E] text-white font-medium px-7 py-3.5 text-xs uppercase tracking-widest transition-all duration-200" style={{ fontFamily: 'Inter, sans-serif' }}>
              <Home className="w-3.5 h-3.5" />
              Go Home
            </Link>
            <Link href="/contact" className="inline-flex items-center justify-center gap-2 border border-white/20 hover:border-white/50 text-white font-medium px-7 py-3.5 text-xs uppercase tracking-widest transition-all duration-200" style={{ fontFamily: 'Inter, sans-serif' }}>
          ...
          <motion.div variants={fadeUp} className="mt-16 pt-10 border-t border-white/10">
            ...
            <div className="flex flex-wrap gap-2 justify-center">
              {[
                ...
                <Link key={link.to} href={link.to} className="text-xs text-slate-400 hover:text-[#C9963B] bg-white/5 hover:bg-white/8 px-4 py-2 border border-white/8 hover:border-[#C9963B]/30 transition-all duration-150 uppercase tracking-wider" style={{ fontFamily: 'Inter, sans-serif' }}>
                  {link.label}
                </Link>
              ))}
            </div>
